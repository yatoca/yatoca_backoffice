...
export default function MuralsComparisonChat({
  ...
  title,
  systemHint,
  endpoint = "/api/murals/chat",
}: {
  basis: any; // analyze result OR compare result
  mode: Mode;
  title?: string;
  systemHint?: string; // optional extra context (e.g. question)
  endpoint?: string;
}) {
  const initialAssistant =
    mode === "compare"
      ? "Listo. Pregúntame lo que quieras sobre la comparación. Puedo explicar diferencias, citar ejemplos y señalar limitaciones."
      : "Listo. Pregúntame lo que quieras sobre el análisis. Puedo resumir temas, emociones, narrativas y citar evidencia.";
  ...
  const headerTitle =
    title ??
    (mode === "compare" ? "Chat (basado en la comparación)" : "Chat (basado en el análisis)");
  ...
  return (
    <div style={containerStyle} onClick={size === "full" ? () => setSize("dock") : undefined}>
      <div style={panelStyle} onClick={(e) => e.stopPropagation()}>
        ...
        >
          <div>
            <div className="fs18 fw700">{headerTitle}</div>
