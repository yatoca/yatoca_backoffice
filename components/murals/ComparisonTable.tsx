...
export default function MuralsComparisonTable({
    ...
    return (
        <div style={{ marginTop: 16, maxWidth: "90vw", overflowX: "auto", paddingBottom: 380 }}>
                        ...
                        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                            {data.source_groups.map((g) => (
                                ...
                                >
                                    ...
                                    {typeof g.total_rows === "number" ? (
                                        <div style={{ fontSize: 12, color: "#666", marginTop: 4 }}>
                                            Frases totales: {g.total_rows}
                                        </div>
                                    ) : null}

                                    {g.top_questions?.length ? (
                                        <div style={{ fontSize: 12, color: "#666", marginTop: 6 }}>
                                            Preguntas más frecuentes:
                                            <ul style={{ marginTop: 6, paddingLeft: 18 }}>
                                                {g.top_questions.slice(0, 3).map((q, idx) => (
                                                    <li key={idx}>
                                                        {q.question} ({q.count})
                                                    </li>
                                                ))}
            ...
            {data?.question_comparability?.length ? (
                <>
                    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #eee", padding: 14 }}>
                        <div className="fs16 fw700">Comparabilidad de preguntas</div>
                        <ul style={{ marginTop: 8, paddingLeft: 18 }}>
                            {data.question_comparability.map((x, i) => (
                                <li key={i} style={{ marginBottom: 6, color: "#333" }}>
                                    {x}
                                </li>
                            ))}
