...
function headerFor(mode: Mode) {
    return mode === "compare" ? "Chat (Radio · comparación)" : "Chat (Radio · análisis)";
}

function starterFor(mode: Mode) {
    return mode === "compare"
        ? "Listo. Pregúntame lo que quieras sobre la comparación. Puedo explicar diferencias entre A y B, citar evidencia y señalar limitaciones."
        : "Listo. Pregúntame lo que quieras sobre el análisis del grupo filtrado. Puedo resumir por grupo, citar evidencia y señalar limitaciones.";
}

export default function RadioComparisonChat({
    basis,
    mode,
}: {
    basis: any;
    ...
    // If mode changes, reset greeting (rare but safe)
    React.useEffect(() => {
    ...
    const send = async () => {
        ...
        // IMPORTANT: build next messages and send THAT to API
